    ...
    private fun cargarVistas() {
        for (integrante in EncuestaSingleton.integrantesEsquemas) {
            if (integrante.tipoCartilla == TipoCartilla.ADOLESCENTE) {
                viewModel.agregarEsquemaVacunacionAdolescente2(EsquemaVacunacionAdolescente2())
            }
        }

        rvEsquemaVacunacionAdolescente2.adapter = context?.let { EsquemaVacunacionAdolescente2Adapter(viewModel.getEsquemasVacunacionAdolescente2(), it) }
