    ...
    private fun cargarVistas() {
        for (integrante in EncuestaSingleton.integrantesEsquemas) {
            if (integrante.tipoCartilla == TipoCartilla.ADULTO_HOMBRE) {
                viewModel.agregarEsquemaVacunacionAdultoHombre2(EsquemaVacunacionAdultoHombre2())
            }
        }

        rvEsquemaVacunacionAdultoHombre2.adapter = context?.let { EsquemaVacunacionAdultoHombre2Adapter(viewModel.getEsquemaVacunacionAdultoHombre2(), it) }
