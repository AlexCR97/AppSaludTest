    ...
    private fun cargarVistas() {
        for (integrante in EncuestaSingleton.integrantesEsquemas) {
            if (integrante.tipoCartilla == TipoCartilla.ADULTO_MUJER) {
                viewModel.agregarEsquemaVacunacionAdultoMujer4(EsquemaVacunacionAdultoMujer4())
            }
        }

        rvEsquemaVacunacionAdultoMujer4.adapter = context?.let { EsquemaVacunacionAdultoMujer4Adapter(viewModel.getEsquemaVacunacionAdultoMujer4(), it) }
