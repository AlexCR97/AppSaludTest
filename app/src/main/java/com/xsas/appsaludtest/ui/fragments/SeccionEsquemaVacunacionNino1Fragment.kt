    ...
    private fun cargarVistas() {
        for (integrante in EncuestaSingleton.integrantesEsquemas) {
            if (integrante.tipoCartilla == TipoCartilla.NINO) {
                viewModel.agregarEsquemaVacunacionNino1(EsquemaVacunacionNino1())
            }
        }

        rvEsquemaVacunacionNino1.adapter = context?.let { EsquemaVacunacionNino1Adapter(viewModel.getEsquemasVacunacionNino1(), it) }
